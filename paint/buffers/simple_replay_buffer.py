...
class SimpleReplayBuffer:
    def __init__(
        ...
        for spec in self._data_specs:
            self._replay_buffer[spec.name] = np.empty(
                (max_size, *spec.shape), dtype=spec.dtype
            )
    ...
    def add_offline_data(self, demos, default_action):
        ...
        # we assume demonstrations contain no stuck states/episodes
        ...
        for idx in range(obs.shape[0]):
            if idx == 0 or term[idx - 1][0]:
                time_step = ExtendedTimeStep(
                    ...
                    reward=0.0,
                    discount=1.0,
                    is_stuck=default_stuck_value,
                )
                self.add(time_step)

            if term[idx][0]:
                time_step = ExtendedTimeStep(
                    observation=next_o[idx],
                    step_type=2,
                    action=acts[idx],
                    reward=rew[idx][0],
                    discount=1.0,
    ...
    def replace(self, idx, time_step):
        for spec in self._data_specs:
            ...
            if spec.name == "discount":
                value = np.expand_dims(time_step.discount * self._discount, 0).astype(
                    "float32"
                )
            ...
            np.copyto(self._replay_buffer[spec.name][idx], value)

        np.copyto(self._replay_buffer["step_type"][idx], time_step.step_type)

    @property
    def stuck_ratio(self):
        assert self._num_transitions > 0

        num_stuck = 0
        for i in range(self._num_transitions):
            num_stuck += self._replay_buffer["is_stuck"][i][0]

        ratio = float(num_stuck) / self._num_transitions
        return ratio

    def add(self, time_step):
        self.replace(time_step=time_step, idx=self._num_transitions)
    ...
    def next(self, batch_size=None, filter_transitions=None, with_replacement=None):
        ...
        filter_transitions = (
            self._filter_transitions
            if filter_transitions is None
            else filter_transitions
        )
        with_replacement = (
            self._with_replacement if with_replacement is None else with_replacement
        )
        ...
        return (
            self._replay_buffer["observation"][idxs - 1],
            ...
            self._replay_buffer["is_stuck"][idxs],
        )
    ...
    def load(self):
        try:
            ...
            self._num_transitions = np.save(
                self._replay_dir / "num_transitions.npy"
            ).tolist()
        except:
