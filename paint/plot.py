...
def make_graph_with_variance(
    ...
    print(
        np.mean(plot_dict[cur_max_index]),
        np.std(plot_dict[cur_max_index]) / np.sqrt(num_seeds),
    )
...
def plotter(experiment_paths, mode, max_index=int(1e8), **plot_config):
    ...
    index, means, stds = make_graph_with_variance(
        y_coords,
        eval_interval,
        max_index=max_index,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--eval_dir",
        type=str,
        default="../earl_benchmark/evaluation/benchmark_evaluation_numbers",
    ...
    plot_config = {
        ...
        "FBRL": {"color": "r", "linestyle": "-", "label": "FBRL", "linewidth": 1.5},
        "naive": {"color": "c", "linestyle": "-", "label": "naive", "linewidth": 1.5},
        "R3L": {"color": "m", "linestyle": "-", "label": "R3L", "linewidth": 1.5},
        "oracle": {
            "color": "#9A9C99",
            "linestyle": "-",
            "label": "oracle",
        ...
        "MEDAL": {"color": "y", "linestyle": "-", "label": "MEDAL", "linewidth": 1.5},
        "MEDAL_ep": {
            "color": "k",
            "linestyle": "-",
            "label": "MEDAL_ep",
    ...
    if plot_type == "tabletop":
        ...
        for method in ["VaPRL", "FBRL", "naive", "R3L", "oracle", "MEDAL"]:
            ...
            if method == "MEDAL":
                ...
                arl_path = os.path.join(
                    os.path.dirname(os.path.abspath(__file__)),
                    "exp_local/tabletop_manipulation",
                    folder_name,
                )
                experiments = []
                for path in os.listdir(arl_path):
                    experiments.append(
                        os.path.join(
                            arl_path,
                            path,
                            next(os.walk(os.path.join(arl_path, path)))[1][0],
                        )
                    )
            else:
                experiment_base = os.path.join(base_path, method.lower())
                experiments = [
                    os.path.join(experiment_base, str(run_id))
        ...
        for method in ["VaPRL", "FBRL", "naive", "R3L", "oracle", "MEDAL", "MEDAL_ep"]:
            ...
            if method in ["MEDAL"]:
                torch_path = os.path.join(
                    os.path.dirname(os.path.abspath(__file__)),
                    "exp_local/sawyer_door/medal_uni_tradrb",
                )
                experiments = []
                for path in os.listdir(torch_path):
                    experiments.append(
                        os.path.join(
                            torch_path,
                            path,
                            next(os.walk(os.path.join(torch_path, path)))[1][0],
                        )
                    )
            elif method in ["MEDAL_ep"]:
                torch_path = os.path.join(
                    os.path.dirname(os.path.abspath(__file__)),
                    "exp_local/sawyer_door/medal_epbuffer",
                )
                experiments = []
                for path in os.listdir(torch_path):
                    experiments.append(
        ...
        for method in ["FBRL", "naive", "R3L", "oracle"]:
            ...
            experiment_base = os.path.join(base_path, method.lower())
            experiments = [
                os.path.join(experiment_base, str(run_id)) for run_id in [0, 1, 2, 3, 4]
            ]
            plotter(experiments, mode=mode, max_index=max_index, **plot_config[method])
