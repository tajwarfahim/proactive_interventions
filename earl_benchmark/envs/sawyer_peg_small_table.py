...
class SawyerPegSmallTableV2(SawyerXYZEnv):
  ...
  def model_name(self):
    return os.path.join(
      os.path.dirname(os.path.realpath(__file__)), "metaworld_assets/sawyer_xyz", 'sawyer_peg_insertion_side_small_table.xml')
...
if __name__ == '__main__':
  import imageio
  env = SawyerPegSmallTableV2(reward_type='sparse')
  env.reset()

  for i in range(100):
    env.step(env.action_space.sample())
    im = env.render('rgb_array')
    imageio.imwrite('sawyer_peg_{}.png'.format(i), im)
