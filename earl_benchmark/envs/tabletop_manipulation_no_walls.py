...
class TabletopManipulationNoWalls(MujocoEnv):
  ...
  def move(self, action):
    ...
    if action[-1] > 0 and not self.dropped_object:
    ...
    next_fist_pos = np.clip(next_fist_pos, -3.8, 3.8)
    if self.attached_object != (-1, -1):
      current_obj_pos = np.array([
          self.sim.data.qpos[i] for i in self.object_dict[self.attached_object]
      ])
      current_obj_pos += (next_fist_pos - current_fist_pos)
      current_obj_pos = np.clip(current_obj_pos, -3.8, 3.8)
